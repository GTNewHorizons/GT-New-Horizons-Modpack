...
public class HazardousItemsHandler
{
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event)
    {
        ...
        {
            // is 250 a good value? I mean, 250ms for ONE LOOP is still pretty
            // insane...
            if (getAverageTiming() > 250)
            {
                ...
                _mExecuteChance += 1;
                _mTimingQueue.clear(); // Reset queue to prevent re-warn on next
                                       // call
            }
            else
            {
    ...
    {
        if (_mTimingQueue == null || _mTimingQueue.isEmpty()) return 0;

        // Only do average calc once we have 20 elements
        if (_mTimingQueue.remainingCapacity() != 0) return 0;
    ...
    public boolean SaveHazardousItems()
    {
        try
        {
            ...
            jaxMarsh.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true); // make
                                                                          // it
                                                                          // nice
                                                                          // and
                                                                          // readable
    ...
    {
        if (_mRnd.nextInt(_mExecuteChance) != 0) return;
        ...
        try
        {
            ...
            if (tUidContact != null && tUidContact.toString() != "minecraft:air")
            {
                ...
                if (hf != null && hf.getCheckContact()) DoHIEffects(hf, pPlayer);
            }
            ...
            if (tUidFeet != null && tUidFeet.toString() != "minecraft:air")
            {
                ...
                if (hi != null && hi.getCheckContact()) DoHIEffects(hi, pPlayer);
            }
        }
        catch (Exception e)
        {
            ...
            for (ItemStack is : tPlayerInventory)
            {
                ...
                // happens here
                {
                    ...
                    if ((Object) is.getItem() instanceof IFluidContainerItem)
                    {
                        ...
                        if (hf != null && hf.getCheckInventory()) DoHIEffects(hf, pPlayer);
                    }
                    ...
                    else
                    {
                        ...
                        if (hi != null && hi.getCheckInventory()) DoHIEffects(hi, pPlayer);
                    }
                }
                catch (Exception e)
                {
