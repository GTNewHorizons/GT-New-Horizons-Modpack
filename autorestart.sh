...
# Pseudo-code:
# IF(minecraft.status == 0)
# Then bash /etc/init.d/minecraft.sh start

# Log into Status.log

# Performance log:
# Performance.sh >> Performance.log
# merge into 1 script, maybe even minecraft.sh
...
mc_disksaverun() {
  if mc_status; then
    ...
    if [ ! -f $MCPATH/$JARFILE ]; then
      ...
      mcwrite "say saving to disk, going read-only..."
      mcwrite "save-off"
      mcwrite "save-all"
      as_user "cp -R $MCPATH/* $MCSTORE/"
      mcwrite "save-on"
      mcwrite "say saving finished, going read/write..."
